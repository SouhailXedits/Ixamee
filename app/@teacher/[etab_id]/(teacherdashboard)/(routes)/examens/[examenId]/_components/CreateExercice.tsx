...
export const CreateExercice: React.FC<CreateExerciceProps> = ({
  allData,
  data,
  setFakeData,
  isArabic,
  ...
  const AddQuestion = () => {
    ...
    setFakeData((prevData: any[]) =>
      prevData.map((item: any) =>
        item.id === data.id ? { ...item, children: [...item.children, newData] } : item
      )
    );
  };

  const deleteExercice = (data: any) => {
    setFakeData(allData.filter((item: any) => item.id !== data.id));
  };

  const handleChangeExcericeName = (e: React.ChangeEvent<HTMLInputElement>) => {
    setFakeData((prevData: any[]) =>
      prevData.map((item: any) =>
        ...
        <div className="p-3 flex items-center justify-between bg-[#F0F6F8] w-full rounded-xl">
          <div className="flex items-center gap-8">
            <Input
              ...
              value={data.name}
              onChange={handleChangeExcericeName}
            />
